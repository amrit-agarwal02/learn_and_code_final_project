...
from server.schemas.news import NewsArticleCreate, NewsArticle, SearchArticleRequest
...
class NewsRepository:
    ...
    def save_news_article_for_user(self, user_id, article_id):
        ...
        cursor.execute(
            """
            INSERT IGNORE INTO saved_article (user_id, article_id)
            VALUES (%s, %s)
            """,
            (
                user_id,
    ...
    def get_article_by_id(self, article_id: int):
        ...
        cursor = conn.cursor(dictionary=True)
        query = """
                    Select a.article_id, title, description, content, source, url, published_at 
                    FROM articles a
                    JOIN article_category_mapping acm ON a.article_id = acm.article_id
                    JOIN category c ON acm.category_id = c.category_id 
                    where
